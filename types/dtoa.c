 ...
 *     returns -1 on failure and 0 otherwise.  soa_dtoa returns NULL
 *     on failure.  soa_strtod indicates failure due to malloc failure
 *     by returning -1.0, setting errno=ENOMEM and *se to s00.
 *
 *  4. The static variable dtoa_result has been removed.  Callers of
 *     soa_dtoa are expected to call soa_freedtoa to free
 *     the memory allocated by soa_dtoa.
 *
 ...
 *     Kmax.
 *
 *  7. soa_strtod has been modified so that it doesn't accept strings with
 *     leading whitespace.
 *
 ***************************************************************/
...
/* struct BCinfo is used to pass information from soa_strtod to bigcomp */
...
/* convert a string s containing nd decimal digits (possibly containing a
   ...
   function carries on where the parsing code in soa_strtod leaves off: on
...
/* Convert a scaled double to a Bigint plus an exponent.  Similar to d2b,
   except that it accepts the scale parameter used in soa_strtod (which
...
/* The bigcomp function handles some hard cases for strtod, for inputs
   ...
   already been obtained by the code in soa_strtod.

   The bigcomp function is only called after soa_strtod has found a
   ...
   Inputs:
     ...
     rv is a (possibly scaled) estimate for the closest double value to the
        value represented by the original input to soa_strtod.  If
        bc->scale is nonzero, then rv/2^(bc->scale) is the approximation to
        the input value.

     bc is a struct containing information gathered during the parsing and
        estimation steps of soa_strtod.  Description of fields follows:
...
soa_strtod(const char *s00, char **se)
{
    ...
    */
    ...
    for(;;) {
        ...
        /* This is the main correction loop for soa_strtod.
...
/* Additional notes (METD): (1) returns NULL on failure.  (2) to avoid memory
   leakage, a successful call to soa_dtoa should always be matched by a
   call to soa_freedtoa. */
