...
Reader::readValue()
{
   ...
   case tokenNull:
      ...
      break;
   default:
       cerr << "token.type_ = " << token.type_ << endl;
...
Reader::readString(Char delim)
{
   ...
   if (c != delim) {
       cerr << "c = " << c << " delim = " << delim << endl;
       cerr << "str was " << string(c2, current_) << endl;
   }
...
Reader::decodeString( Token &token, std::string &decoded )
{
   ...
   Location current = token.start_;
   Char delim = *current++;
   Location end = token.end_ - 1;      // do not include delim
   while ( current != end )
   {
      Char c = *current++;
      if ( c == delim )
