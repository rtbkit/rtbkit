	...
	Pvoid_t	Pjpm)
{
	...
	PjblRaw	= (Pjbl_t)j__udyAllocJBL((Pjpm_t)Pjpm);
...
//	Get memory for a Bitmap branch
	PjbbRaw	= (Pjbb_t)j__udyAllocJBB((Pjpm_t)Pjpm);
...
//			Get number of JPs in this sub expanse
			...
			PjpRaw = (Pjp_t)j__udyAllocJBBJP(NumJP, (Pjpm_t)Pjpm);
                        Pjp    = P_JP(PjpRaw);
...
// Free any previous allocations:
				...
				if (NumJP)
				{
				    j__udyFreeJBBJP(JU_JBB_PJP(Pjbb,
						    CurrSubExp), NumJP, (Pjpm_t)Pjpm);
				}
			    }
			    j__udyFreeJBB(PjbbRaw, (Pjpm_t)Pjpm);
			    return(-1);
			}
...
// Allocate memory for a BranchU:
	...
	PjbuRaw = j__udyAllocJBU((Pjpm_t)Pjpm);
...
// Free the subexpanse:
		...
		j__udyFreeJBBJP(JU_JBB_PJP(Pjbb, ii), jj, (Pjpm_t)Pjpm);
