...
BOOST_AUTO_TEST_CASE( test_mopub )
{
    ...
    // our firewall.
    std::shared_ptr<MoPubExchangeConnector> connector
    (new MoPubExchangeConnector("connector", proxies));
    ...
    for (auto & c: agent.config.creatives) {
        c.providerConfig["mopub"]["attr"] = "11";
        c.providerConfig["mopub"]["type"] = "5";
        ...
        c.providerConfig["mopub"]["adm"]
            = "<img src=\"http://dsp.com/creative.png?width="
              ...
              + std::to_string(c.format.height)
              + "&price=${AUCTION_PRICE}\"/>";
        c.providerConfig["mopub"]["iurl"] = "www.iurl.com";
    }
