...
bool bit_equal(float f1, float f2)
{
    return (f1 < f1 && f2 < f2)
        || f1 == f2;
}
...
bucket_dist_full(vector<float> & result)
{
    ...
    if (debug) {
        for (Freqs::const_iterator it = freqs.begin();  it != freqs.end();
             ++it)
...
bucket_dist_reduced(vector<float> & result, size_t num_buckets)
{
    ...
    */
    ...
    for (Freqs::const_iterator it = freqs.begin();
         it != freqs.end();  ++it) {
        float n = it->second;
        if (n >= per_bucket) {
...
create_buckets(size_t num_buckets)
{
    ...
    if (num_buckets >= freqs.size())
        bucket_dist_full(result.splits);
    else
        bucket_dist_reduced(result.splits, num_buckets);
