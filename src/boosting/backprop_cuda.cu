                  ...
                  float * layer_outputs)  // global scratch space[total neurons]
{
    ...
    float * this_layer_outputs = layer_outputs;
    size_t layer_weights = 0;  // offset in texture
    size_t layer_biases  = 0;  // offset in texture
    ...
    for (unsigned l = 0;
...
#if 0
        // Start off with the bias terms
        double accum = (tid < no ? tex1Dfetch(biases_tex, layer_biases + tid) : 0.0);
        ...
        if (__any(tid < no)) {
            ...
            for (unsigned i = 0;  i < ni;  ++i) {
                ...
                // Coalesced access; maybe texture would be better
                float weight
                    = (tid < no ? tex1Dfetch(weights_tex, layer_weights + i * w_stride + tid) : 0.0);
        ...
        */
        ...
        for (unsigned i = idx;  i < ni;  i += o_threads) {
            // warning: bank conflicts...
            float inval = scratch[i];
            float weight = tex1Dfetch(weights_tex, layer_weights + i * w_stride + o);
        ...
        if (max_threads > 1) {
            ...
            if (tid < no) scratch[tid] = tex1Dfetch(biases_tex, layer_biases + tid);
            ...
            accum += tex1Dfetch(biases_tex, layer_biases + o);
            this_layer_outputs[o]
                = scratch[o]
                = transform(accum, activation);
    ...
    for (int l = num_layers - 1;  l >= 0;
        ...
        if (l > 0) {
            ...
            if (tid < ni) {
                for (unsigned o = 0;  o < no;  ++o) {
                    float d = scratch[o];  // may be the d from another thread
                    float update = d * tex1Dfetch(weights_tex, layer_weights + tid * w_stride + o);
                    total += update;
                }
            }
...
__global__ void
train_examples_kernel_tex(const float * feature_vectors,  // feature vector [ni]
                          int feature_vector_width,
                          const int * labels,
                          const float * example_weights,
                          int num_layers,
                          ...
                          int max_width)
{
    ...
    */
    ...
    for (;  example_num < last_example;  ++example_num) {
        ...
        train_example_tex(input, label, example_weight,
                      num_layers, scratch, architecture, w_strides,
                      w_updates, b_updates,
                      activation, fire, inhibit, learning_rate,
                      num_threads_in_block,
                      num_threads_on_multiprocessor,
...
struct Backprop::Context {
    ...
    void execute()
    {
        if (plan.use_textures) {

            train_examples_kernel_tex<<<grid, plan.threads, plan.shared_mem_size>>>
                (d_feature_vectors,
