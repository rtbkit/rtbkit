...
__global__ void
stumpBinsymKernel(const uint16_t * buckets,
...
__global__ void
stumpBinsymKernelPacked(const uint64_t * index_data,
                        shift_t bucket_bits,
                        shift_t example_bits,
                        shift_t label_bits,
                        shift_t divisor_bits,
                        uint32_t size,
                        const float * weights,
                        const float * ex_weights,
                        TwoBuckets * buckets_global,
                        ...
                        bool use_texture)
{
    ...
    if (example_bits == 0) {
        ...
        // We modify so that 4 values are done by tid 0, then 4 by tid 1,
        // and so on so that our memory accesses really read 4 values.
        start_at = offset + (tid * 4);
        index.advance(start_at * total_bits);

        for (unsigned i = 0;  i < num_todo;  i += 4, index.advance((4 * num_threads - 4) * total_bits)) {
            int example = start_at + i * num_threads;
            ...
            unsigned label;
            float divisor;
            unsigned real_bucket;
            
            float to_add;
            int bucket;

            real_bucket = index.extract<unsigned>(bucket_bits);
            label = index.extract<unsigned>(label_bits);
            if (divisor_bits == 0) divisor = 1.0;
            else divisor = index.extract<float>(divisor_bits);

#if 0
            // First update (x)
            index.extract(real_bucket, bucket_bits,
                          label, label_bits,
                          divisor, divisor_bits);
#endif

            to_add = weight.x * divisor;
            bucket = real_bucket * bucket_expansion + expansion_offset;
            atomic_add_shared(buckets_shared[bucket][label], to_add);
            if (label) w_label_true += to_add;
            else w_label_false += to_add;
            if (example + 1 == size) break;
        ...
        for (unsigned n = 0;  n < num_todo;  ++n, index.advance((num_threads - 1) * total_bits)) {
            int i = start_at + n * num_threads;
            ...
            else example = index.extract<float>(example_bits);
            label = index.extract<unsigned>(label_bits);
            if (divisor_bits == 0) divisor = 1.0;
            else divisor = index.extract<float>(divisor_bits);
...
struct Test_Buckets_Binsym::Plan {
    ...
    Plan(const uint16_t * buckets,
        ...
        : buckets(buckets), examples(examples), labels(labels),
          ...
          on_device(on_device), compressed(compressed)
          
    {
        ...
        if (compressed)
            compressed_index.init(buckets, examples, labels, divisors, size);
        ...
        //
        // We need:
        // - sizeof(TwoBuckets) for each bucket;
        // - sizeof(TwoBuckets) for the total
        //
        ...
        // parallelism.
        shared_mem_size = sizeof(TwoBuckets) * (buckets_to_allocate + 1);
        
        cerr << "shared_mem_size = " << shared_mem_size << endl;
        
        if (compressed) {
            d_compressed_index.init(compressed_index.data.get(),
                                    compressed_index.num_words);
        }
        else {
    ...
    Bit_Compressed_Index compressed_index;
    DeviceData<uint64_t> d_compressed_index;
                ...
                TwoBuckets & w_label) const
    {
        ...
        if (compressed) {
            CUDA::executeHostCompressed(accum, w_label, weights, ex_weights,
                                        compressed_index.data.get(),
                                        compressed_index.bucket_bits,
                                        compressed_index.example_bits,
                                        compressed_index.label_bits,
                                        compressed_index.divisor_bits,
                                        size);
        }
        else {
            CUDA::executeHost(accum, w_label, weights, ex_weights,
                              buckets, examples, labels, divisors,
                              size);
        }
                  ...
                  TwoBuckets & w_label) const
    {
        ...
        // execute the kernel
        if (compressed)
            stumpBinsymKernelPacked<<< grid, threads, shared_mem_size >>>
                ( d_compressed_index,
                  compressed_index.bucket_bits,
                  compressed_index.example_bits,
                  compressed_index.label_bits,
            ...
            stumpBinsymKernel<<< grid, threads, shared_mem_size >>>
     ...
     bool compressed) const
{
    return boost::shared_ptr<Test_Buckets_Binsym::Plan>
        (new Plan(buckets, examples, labels, divisors, size, weights,
                  ex_weights, num_buckets, on_device, compressed));
}
