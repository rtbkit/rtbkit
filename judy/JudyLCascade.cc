...
// Allocate the LeafB1:
    ...
    if ((PjlbRaw = (Pjlb_t)j__udyAllocJLB1((Pjpm_t)Pjpm)) == (Pjlb_t) NULL)
	    return((Pjlb_t) NULL);
	Pjlb = P_JLB(PjlbRaw);
...
// TBD:  There is an opportunity to put pop == 1 value in pointer:
		...
		pv[subexp].pv_Pjv
                    = j__udyLAllocJV((Word_t)pv[subexp].pv_Pop1, (Pjpm_t)Pjpm);
...
// Upon out of memory, free all previously allocated:
		...
		if (pv[subexp].pv_Pjv == (Pjv_t) NULL)
		{
		    ...
		    j__udyFreeJLB1(PjlbRaw, (Pjpm_t)Pjpm);
		    return((Pjlb_t) NULL);
		}
...
JUDYLCODE(Pjv_t	   Pjv;)
	...
	PjlbRaw = j__udyAllocJLB1((Pjpm_t)Pjpm);
...
//	    Get number of Indexes in subexpanse
		...
		PjvnewRaw = j__udyLAllocJV(Pop1, (Pjpm_t)Pjpm);
...
//                  Free prevously allocated LeafVs:
			...
			if ((Pop1 = j__udyCountBitsL(JU_JLB_BITMAP(Pjlb, ii))))
			{
			    ...
			    j__udyLFreeJV(PjvnewRaw, Pop1, (Pjpm_t)Pjpm);
			}
		    }
//                  Free the bitmap leaf
		    j__udyLFreeJLB1(PjlbRaw, (Pjpm_t)Pjpm);
		    return(-1);
		}
...
//				Allocate Value area for Immediate Leaf
				...
				if (PjvnewRaw == (Pjv_t) NULL)
					FREEALLEXIT(ExpCnt, StageJP, (Pjpm_t)Pjpm);
...
//				Get a new Leaf
				PjllRaw = j__udyAllocJLL1(Pop1, (Pjpm_t)Pjpm);
...
//      Now put all the Leaves below a BranchL or BranchB:
	if (ExpCnt <= cJU_BRANCHLMAXJPS) // put the Leaves below a BranchL
	{
	    if (j__udyCreateBranchL(Pjp, StageJP, StageExp, ExpCnt,
			Pjpm) == -1) FREEALLEXIT(ExpCnt, StageJP, (Pjpm_t)Pjpm);

	    Pjp->jp_Type = cJU_JPBRANCH_L2;
	}
	else
	{
	    if (j__udyStageJBBtoJBB(Pjp, &StageJBB, StageJP, SubJPCount, (Pjpm_t)Pjpm)
		== -1) FREEALLEXIT(ExpCnt, StageJP, (Pjpm_t)Pjpm);
	}
	return(1);
...
//		Alloc a 2 byte Index Leaf
		PjllRaw	= j__udyAllocJLL2(cJU_LEAF3_MAXPOP1, (Pjpm_t)Pjpm);
...
//		cJU_JPLEAF2
		      ...
		      JUDYLCODE(Pjv_t  Pjvnew;)	 // value area of new leaf.
				...
				PjllRaw = j__udyAllocJLL2(Pop1, (Pjpm_t)Pjpm);
				if (PjllRaw == (Pjll_t) NULL)
					FREEALLEXIT(ExpCnt, StageJP, (Pjpm_t)Pjpm);
...
//		Alloc a 3 byte Index Leaf
		PjllRaw = j__udyAllocJLL3(cJU_LEAF4_MAXPOP1, (Pjpm_t)Pjpm);
		if (PjllRaw == (Pjlb_t)NULL) return(-1);  // out of memory
...
//		cJU_JPLEAF3
		      ...
		      JUDYLCODE(Pjv_t  Pjvnew;)	 // value area of new leaf.
				...
				PjllRaw = j__udyAllocJLL3(Pop1, (Pjpm_t)Pjpm);
				if (PjllRaw == (Pjll_t)NULL)
					FREEALLEXIT(ExpCnt, StageJP, (Pjpm_t)Pjpm);
...
//		Alloc a 4 byte Index Leaf
		PjllRaw = j__udyAllocJLL4(cJU_LEAF5_MAXPOP1, (Pjpm_t)Pjpm);
		if (PjllRaw == (Pjlb_t)NULL) return(-1);  // out of memory
...
//				Get a new Leaf
				PjllRaw = j__udyAllocJLL4(Pop1, (Pjpm_t)Pjpm);
				if (PjllRaw == (Pjll_t)NULL)
					FREEALLEXIT(ExpCnt, StageJP, (Pjpm_t)Pjpm);
...
//		Alloc a 5 byte Index Leaf
		PjllRaw = j__udyAllocJLL5(cJU_LEAF6_MAXPOP1, (Pjpm_t)Pjpm);
		if (PjllRaw == (Pjlb_t)NULL) return(-1);  // out of memory
...
//				Get a new Leaf
				PjllRaw = j__udyAllocJLL5(Pop1, (Pjpm_t)Pjpm);
				if (PjllRaw == (Pjll_t)NULL)
					FREEALLEXIT(ExpCnt, StageJP, (Pjpm_t)Pjpm);
...
//		Alloc a 6 byte Index Leaf
		PjllRaw = j__udyAllocJLL6(cJU_LEAF7_MAXPOP1, (Pjpm_t)Pjpm);
		if (PjllRaw == (Pjlb_t)NULL) return(-1);  // out of memory
...
//				Get a new Leaf
				PjllRaw = j__udyAllocJLL6(Pop1, (Pjpm_t)Pjpm);
				if (PjllRaw == (Pjll_t)NULL)
					FREEALLEXIT(ExpCnt, StageJP, (Pjpm_t)Pjpm);
				Pjll = P_JLL(PjllRaw);
...
#ifdef JU_64BIT
		PjllRaw	= j__udyAllocJLL7(cJU_LEAFW_MAXPOP1, (Pjpm_t)Pjpm);
		if (PjllRaw == (Pjlb_t)NULL) return(-1);  // out of memory
...
#else // 32 Bit
		PjllRaw	= j__udyAllocJLL3(cJU_LEAFW_MAXPOP1, (Pjpm_t)Pjpm);
...
//		cJU_JPLEAF3[7]
		      ...
		      JUDYLCODE(Pjv_t  Pjvnew;)	 // value area of new leaf.
#ifdef JU_64BIT
				PjllRaw = j__udyAllocJLL7(Pop1, (Pjpm_t)Pjpm);
				if (PjllRaw == (Pjll_t) NULL) return(-1);
				Pjll = P_JLL(PjllRaw);
