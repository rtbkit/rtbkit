...
// Allocate and populate the BranchL:
	...
	if ((PjblRaw = j__udyAllocJBL((Pjpm_t)Pjpm)) == (Pjbl_t) NULL) return(-1);
	Pjbl = P_JBL(PjblRaw);
...
// Copy JPs from each BranchB subexpanse subarray:
	...
	for (SubExp = 0; SubExp < cJU_NUMSUBEXPB; ++SubExp)
	{
	    ...
	    Pjpjbl += NumJPs;
	    j__udyFreeJBBJP(PjpRaw, NumJPs, (Pjpm_t)Pjpm);	// subarray.
	}
	j__udyFreeJBB(PjbbRaw, (Pjpm_t)Pjpm);		// BranchB itself.
...
// Allocate JPLEAF1 and prepare pointers:
	...
	if ((PjllRaw = j__udyAllocJLL1(cJU_LEAF1_MAXPOP1, (Pjpm_t)Pjpm)) == 0)
	    return(-1);
...
// Copy all old-LeafB1 value areas from value subarrays to new Leaf1:
	...
	for (SubExp = 0; SubExp < cJU_NUMSUBEXPL; ++SubExp)
	{
	    ...
	    j__udyLFreeJV(PjvRaw, Pop1, (Pjpm_t)Pjpm);
...
// Copy all old-LeafB1 value areas from value subarrays to new Leaf2:
		...
		j__udyLFreeJV(Pjv1Raw, SubExpPop1, (Pjpm_t)Pjpm);
		Pjv2 += SubExpPop1;			// advance through new.
	    }
#endif // JUDYL

	    j__udyFreeJLB1((Pjlb_t) (Pjp->jp_Addr), (Pjpm_t)Pjpm);  // LeafB1 itself.
	    return(Pop1);
...
// (plus, for JudyL, value areas):
	    ...
	    j__udyFreeJLL1((Pjll_t) (Pjp->jp_Addr), Pop1, (Pjpm_t)Pjpm);
	    return(Pop1);
	}
#endif // (JUDYL || (! JU_64BIT))
...
// JPIMMED_1_0[2+]:
...
#endif
	{
	    ...
	    JUDYLCODE(j__udyLFreeJV(Pjv1Raw, Pop1, (Pjpm_t)Pjpm);)
	    return(Pop1);
	}
...
#endif
	    j__udyFreeJLL2((Pjll_t) (Pjp->jp_Addr), Pop1, (Pjpm_t)Pjpm);
	    return(Pop1);
	}
...
// JPIMMED_2_0[2+]:
...
#if (defined(JUDY1) || defined(JU_64BIT))
	{
	    ...
	    Pjv2    = P_JV(Pjv2Raw);
	    JU_COPYMEM(Pjv3, Pjv2, Pop1);
	    j__udyLFreeJV(Pjv2Raw, Pop1, (Pjpm_t)Pjpm);
#endif
	    return(Pop1);
	}
...
#endif
	    j__udyFreeJLL3((Pjll_t) (Pjp->jp_Addr), Pop1, (Pjpm_t)Pjpm);
	    return(Pop1);
	}
...
// JPIMMED_3_0[2+]:
...
#endif
	{
	    ...
	    Pjv3    = P_JV(Pjv3Raw);
	    JU_COPYMEM(Pjv4, Pjv3, Pop1);
	    j__udyLFreeJV(Pjv3Raw, Pop1, (Pjpm_t)Pjpm);
#endif
	    return(Pop1);
	}
...
#endif
	    j__udyFreeJLL4((Pjll_t) (Pjp->jp_Addr), Pop1, (Pjpm_t)Pjpm);
	    return(Pop1);
	}
...
#endif
	    j__udyFreeJLL5((Pjll_t) (Pjp->jp_Addr), Pop1, (Pjpm_t)Pjpm);
	    return(Pop1);
	}
...
#endif
	    j__udyFreeJLL6((Pjll_t) (Pjp->jp_Addr), Pop1, (Pjpm_t)Pjpm);
	    return(Pop1);
	}
...
#endif
	    j__udyFreeJLL7((Pjll_t) (Pjp->jp_Addr), Pop1, (Pjpm_t)Pjpm);
	    return(Pop1);
	}
